...
const I18N = {
    de: {
        toast_welcome_title: "QR-Generator geladen!",
        toast_welcome_message: "Erstelle jetzt deinen Minecraft-QR-Code!",
        ...
        hero_title: "DEINE <span class=\"highlight\">MOB QR</span> PLATTFORM",
    ...
    en: {
        toast_welcome_title: "QR Generator loaded!",
        toast_welcome_message: "Create your Minecraft QR code now!",
        site_title: "MC-Craft | QR Generator",
        site_title_short: "MC-Craft",
        ...
        hero_title: "YOUR <span class=\"highlight\">MOB QR</span> PLATFORM",
...
const mobCatalogFiles = [
    ...
    { category: 'jockeys', file: 'Jockeys_mobs.json' },
...
function renderAssetOptions(preferredValue = '') {
    ...
    if (selectedType === 'mob') {
        ...
        for (const file of mobCatalogFiles) {
            ...
            group.label = categoryLabels[lang]?.[category] || category;
        ...
        for (const file of itemCatalogFiles) {
            ...
            const group = document.createElement('optgroup');
            group.label = itemCategoryLabels[lang]?.[file.category] || file.category;
            groupEntries
                .slice()
...
async function loadMobCatalogOptions() {
    ...
    for (const file of mobCatalogFiles) {
        const mobs = await loadJsonArray(`/assets/JS/mobs/${langPrefix}${file.file}`);
    ...
    for (const file of itemCatalogFiles) {
        const entries = await loadJsonArray(`/assets/JS/items/${langPrefix}${file.file}`);
        if (loadId !== activeMobCatalogLoad) return;
