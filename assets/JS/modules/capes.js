...
function getCapeName(cape) {
    return currentLang === 'de' ? (cape.name_de || cape.name) : (cape.name_en || cape.name);
}

function getCapeDescription(cape) {
    return currentLang === 'de' ? (cape.description_de || cape.description) : (cape.description_en || cape.description);
}

function getCapeTags(cape) {
    return currentLang === 'de' ? (cape.tags_de || cape.tags) : (cape.tags_en || cape.tags);
}

function getAnimationModeLabel(mode) {
    switch (mode) {
        case 'run': return getTranslation('capes_anim_run');
        case 'fly': return getTranslation('capes_anim_fly');
        case 'swim': return getTranslation('capes_anim_swim');
        case 'sneak': return getTranslation('capes_anim_sneak');
        default: return getTranslation('capes_anim_walk');
    }
}
...
function updateLanguage() {
    ...
    document.title = getTranslation('site_title_capes');

    // Rotations-Status
    const rotateStatus = document.getElementById('rotateStatus');
    if (rotateStatus) {
        rotateStatus.textContent = isRotating ? getTranslation('capes_rotate_on') : getTranslation('capes_rotate_off');
    }
    if (animationModeSelect) {
        animationModeSelect.value = animationMode;
    }
...
function initLoader() {
    ...
    const texts = [
        getTranslation('loader_text1_capes'),
...
function showWelcomeToast() {
    if (allCapes && allCapes.length) {
        showToast(
            ...
            `${allCapes.length} ${getTranslation('capes_filter_all')}`,
            'success'
        );
    }
}
...
async function loadAllCapesData() {
    ...
    try {
        ...
        showError(getTranslation('toast_error_no_data'));
    }
}
    ...
    if (capesGrid) {
        capesGrid.innerHTML = `
            <div class="error-message" style="text-align:center; padding:2rem;">
                ...
                <h3>${getTranslation('toast_error_title')}</h3>
                <p>${message}</p>
                <button onclick="location.reload()" class="btn btn-primary">
                    <i class="fas fa-redo"></i> ${getTranslation('loader_text1_capes')}
                </button>
            </div>
        `;
    }
...
function initCapeGallery() {
    if (!allCapes.length) {
        ...
        showToast(getTranslation('toast_error_title'), getTranslation('toast_error_no_data'), 'error');
        return;
    }
    ...
    if (!capes.length) {
        capesGrid.innerHTML = `
            <div class="no-results">
                ...
                <h3>${getTranslation('capes_filter_all')}</h3>
                <p>${getTranslation('capes_gallery_subtitle')}</p>
            </div>
        `;
        return;
    }
...
function selectCape(cape) {
    ...
    try {
        ...
        showToast(getTranslation('toast_cape_loaded'), getCapeName(cape), 'success');
    } catch (error) {
        console.error('Fehler beim Laden des Capes:', error);
        showToast(getTranslation('toast_error_title'), getTranslation('toast_download_failed'), 'error');
    }
}
...
function updateCapeInfo(cape) {
    ...
    if (capeAvailabilityEl) {
        capeAvailabilityEl.textContent = cape.availability || getTranslation('capes_availability_official');
    }
}
...
function updateCustomElytraInfo() {
    ...
    capeTypeEl.textContent = getTranslation('capes_type_custom_elytra');
    if (capeAvailabilityEl) {
        capeAvailabilityEl.textContent = getTranslation('capes_availability_custom');
    }
}
...
function updateViewModeButtonState() {
    if (viewModeStatus) {
        viewModeStatus.textContent = getTranslation(
...
function toggleElytraMode() {
    ...
    showToast(
        getTranslation('toast_view_mode_title'),
        getTranslation(isElytraMode ? 'toast_view_mode_elytra_active' : 'toast_view_mode_cape_active'),
        'info'
    );
}
...
function handleElytraUpload(file) {
    ...
    reader.onload = (event) => {
        ...
        showToast(
            getTranslation('toast_upload_success_elytra'),
            file.name,
            'success'
        );
    };
    ...
    reader.onerror = () => {
        showToast(
            getTranslation('toast_error_title'),
            getTranslation('toast_upload_error_elytra'),
            'error'
        );
    };
...
function toggleRotation() {
    ...
    status.textContent = isRotating ? getTranslation('capes_rotate_on') : getTranslation('capes_rotate_off');
    showToast(
...
function toggleAnimation() {
    ...
    toggleAnimButton.innerHTML = isAnimating 
        ? '<i class="fas fa-pause"></i> ' + getTranslation('capes_btn_pause')
        : '<i class="fas fa-play"></i> ' + getTranslation('capes_btn_play') || 'Animation starten';
    showToast(
...
function updateLayerButtonState() {
    if (layerStatus) {
        layerStatus.textContent = secondLayerVisible ? getTranslation('capes_rotate_on') : getTranslation('capes_rotate_off');
    }
    if (toggleLayerBtn) {
...
function downloadCurrentCape() {
    ...
    if (!currentCape) {
        showToast(
            getTranslation('toast_error_title'), 
            getTranslation('toast_error_no_cape'), 'error'
        );
        return;
    }
    const a = document.createElement('a');
...
window.addEventListener('error', (e) => {
    ...
    if (!document.querySelector('.error-toast')) {
        showToast(getTranslation('toast_error_title'), getTranslation('toast_error_general'), 'error');
    }
});

window.addEventListener('online', () => showToast(getTranslation('toast_online_title'), getTranslation('toast_online_message'), 'success'));
window.addEventListener('offline', () => showToast(getTranslation('toast_offline_title'), getTranslation('toast_offline_message'), 'warning'));
