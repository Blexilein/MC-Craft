...
function updateLanguage() {
    ...
    document.title = getTranslation('coming_soon_title') + ' | MC-Craft';
}
...
function initNotify() {
    ...
    btn.addEventListener('click', () => {
        ...
        if (!email || !email.includes('@')) {
            showToast(getTranslation('toast_error'), 'Bitte eine gültige E‑Mail eingeben.', 'error');
            input.style.borderColor = '#FF6B6B';
            setTimeout(() => input.style.borderColor = '', 1500);
            return;
        }
        // Hier könnte eine API‑Anfrage erfolgen – wir simulieren Erfolg
        showToast(getTranslation('toast_success'), getTranslation('notify_success'));
        if (successMsg) {
            successMsg.textContent = getTranslation('notify_success');
