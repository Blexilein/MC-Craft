...
function formatGenericValue(value) {
    ...
    if (typeof value === 'object') {
        ...
        return entries.length ? entries.map(([k, v]) => `${humanizeKey(k)}: ${v}`).join(' / ') : getTranslation('value_none');
    }
    return String(value);
}
...
function formatYesNo(value) {
    if (typeof value === 'boolean') return value ? getTranslation('yes') : getTranslation('no');
    if (value === undefined || value === null || value === '') return getTranslation('value_none');
    return value;
}

function formatDrops(drops) {
    if (drops === undefined || drops === null) return getTranslation('value_none');
    if (Array.isArray(drops)) {
        if (!drops.length) return getTranslation('value_none');
    ...
    return drops || getTranslation('value_none');
}
...
function renderExtraItemFields(item) {
    ...
    modalItemExtraGrid.innerHTML = rows.map(f => `
        <div class="property">
            <div class="property-label">${escapeHtml(f[currentLang] || f.de)}</div>
...
async function loadAllItemsData() {
    ...
    console.log(`Gesamt: ${total} Items (${currentLang})`);
...
function getCategoryName(category) {
    const names = {
        building: getTranslation('filter_building'),
        color: getTranslation('filter_color'),
        nature: getTranslation('filter_nature'),
        utility: getTranslation('filter_utility'),
        redstone: getTranslation('filter_redstone'),
        tools: getTranslation('filter_tools'),
        combat: getTranslation('filter_combat'),
        food: getTranslation('filter_food'),
        materials: getTranslation('filter_materials'),
        spawn: getTranslation('filter_spawn'),
        gamemod: getTranslation('filter_gamemod')
    };
    return names[category] || category;
}
function updateItemStats() {
    if(itemCount) itemCount.textContent = `${state.filteredItems.length} ${getTranslation('items_grid_count').replace(/[0-9+]/g, '').trim()}`;
...
function openItemModal(item) {
    ...
    document.body.style.overflow = 'hidden';
    showToast(getTranslation('toast_item_opened'), getTranslation('toast_item_opened_message', { name: item.name }), 'info');
}
...
function setModalProperties(item) {
    ...
    modalItemDescription.textContent = item.description || getTranslation('no_description');
    if (modalItemDrops) {
...
function setModalExtendedInfo(item) {
    ...
    modalItemEdition.textContent = item.edition || getTranslation('value_none');
    modalItemStatus.textContent = item.status || getTranslation('value_none');
    modalItemRarity.textContent = item.rarity || getTranslation('value_none');
    modalItemVariant.textContent = item.variant || getTranslation('value_none');
    modalItemIntroduced.textContent = item.introduced_date || getTranslation('value_none');
    modalItemRemoved.textContent = item.removed_date || getTranslation('value_none');
    modalItemUpdated.textContent = item.updated_at || getTranslation('value_none');
    ...
    if (modalItemEffectsContent) {
        ...
        html += `<p><strong>${escapeHtml(getTranslation('effects_gives'))}:</strong> ${item.effect_immunities?.length ? escapeHtml(item.effect_immunities.join(', ')) : getTranslation('value_none')}</p>`;
    ...
    if (modalItemMiscContent) {
        const rows = [
            [getTranslation('misc_aliases'), item.aliases?.length ? item.aliases.join(', ') : null, false],
            [getTranslation('misc_tags'), item.tags?.length ? item.tags.join(', ') : null, false],
            [getTranslation('misc_obtain'), item.obtain?.length ? item.obtain.join(', ') : null, false],
            [getTranslation('misc_crafting_recipe'), (item.crafting_recipe && typeof item.crafting_recipe === 'object' && Object.keys(item.crafting_recipe).length) ? formatGenericValue(item.crafting_recipe) : (typeof item.crafting_recipe === 'string' ? item.crafting_recipe : null), false],
            [getTranslation('misc_enchantments'), item.enchantments?.length ? item.enchantments.join(', ') : null, false],
            [getTranslation('misc_incompatible_enchantments'), item.incompatible_enchantments?.length ? item.incompatible_enchantments.join(', ') : null, false],
            [getTranslation('misc_trade_info'), (item.trade_info && (!Array.isArray(item.trade_info) || item.trade_info.length)) ? formatGenericValue(item.trade_info) : null, false],
            [getTranslation('misc_block_states'), item.block_states?.length ? item.block_states.join(', ') : null, false],
            [getTranslation('misc_related'), item.related_entries?.length ? item.related_entries.join(', ') : null, true],
            [getTranslation('misc_java_id'), item.java_item_id || item.java_block_id || null, true],
            [getTranslation('misc_bedrock_id'), item.bedrock_item_id || item.bedrock_block_id || null, true]
        ];
        ...
        modalItemMiscContent.innerHTML += item.source
            ? `<p><strong>${escapeHtml(getTranslation('misc_source'))}:</strong> <a href="${escapeHtml(item.source)}" target="_blank" rel="noopener">${escapeHtml(item.source)}</a></p>`
    ...
    if (modalItemNotes) {
        modalItemNotes.textContent = item.notes || getTranslation('value_none');
    }

    // Fun Fact (immer sichtbar)
    if (modalItemFunFact) {
        modalItemFunFact.textContent = item.fun_fact || getTranslation('value_none');
    }
}

function getDisplayTextureMeta(texture) {
    return {
        name: texture?.name || getTranslation('texture_unnamed'),
...
function setModalTexture(item) {
    ...
    if (!textures.length) {
        modalItemTexturesContainer.innerHTML = `<p class="no-texture-message" data-i18n="modal_no_texture">${getTranslation('modal_no_texture')}</p>`;
