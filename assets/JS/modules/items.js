...
const I18N = {
    de: {
        ...
        disc_title: "Titel",
        disc_artist: "Künstler",
        disc_length: "Länge",
        disc_released_in: "Veröffentlicht in",
        disc_release_date: "Erscheinungsdatum",
        disc_comparator: "Komparator-Signal",
        disc_sound_event: "Sound-Event",
        disc_youtube: "Auf YouTube suchen",
        copy_all: "Alle kopieren",
        copied_all: "Alle kopiert!",
        items_no_results_title: "Keine Items gefunden",
        items_no_results_desc: "Versuche einen anderen Suchbegriff oder eine andere Kategorie"
    },
    en: {
        ...
        disc_artist: "Artist",
        disc_length: "Length",
        disc_released_in: "Released in",
        disc_release_date: "Release date",
        disc_comparator: "Comparator signal",
        disc_sound_event: "Sound event",
        disc_youtube: "Search on YouTube",
        copy_all: "Copy all",
        copied_all: "All copied!",
        items_no_results_title: "No items found",
        items_no_results_desc: "Try a different search term or another category"
    }
};
// ===== DOM ELEMENTE =====
...
function renderExtraItemFields(item) {
    ...
    modalItemExtraGrid.innerHTML = rows.map(f => `
        <div class="property">
            <div class="property-label">${escapeHtml(f[lang] || f.de)}</div>
...
async function loadAllItemsData() {
    ...
    console.log(`Total: ${total} items (${lang})`);
...
function loadFallbackItems() {
    ...
    itemsData.building = fallback.building;
    itemsData.tools = fallback.tools;
    itemsData.combat = fallback.combat;
...
function renderItems() {
    ...
    if(!state.filteredItems.length) {
        ...
        <div class="no-results">
            ...
            </i><h3>${t('items_no_results_title')}</h3>
            <p>${t('items_no_results_desc')}</p>
        </div>`;
        return;
    }
...
// ===== MUSIC DISC INFO BOX =====
function formatDiscLength(seconds) {
    return formatSoundTime(Math.round(Number(seconds) || 0));
}

function formatReleaseDate(iso) {
    const date = new Date(`${iso}T12:00:00`);
    if (!iso || Number.isNaN(date.getTime())) return iso || t('value_unknown');
    return date.toLocaleDateString(lang === 'de' ? 'de-DE' : 'en-US', { day: 'numeric', month: 'long', year: 'numeric' });
}

function renderDiscInfo(item) {
    const box = document.getElementById('modalItemDiscInfo');
    if (!box) return;
    const disc = item.music_disc;
    box.hidden = !disc;
    if (!disc) {
        box.innerHTML = '';
        return;
    ...
    const rows = [
        [t('disc_title'), escapeHtml(disc.title)],
        [t('disc_artist'), escapeHtml(disc.artist || t('value_unknown'))],
        [t('disc_length'), formatDiscLength(disc.length_seconds)],
        [t('disc_released_in'), escapeHtml(disc.released?.version || t('value_unknown'))],
        [t('disc_release_date'), escapeHtml(formatReleaseDate(disc.released?.date))],
        [t('disc_comparator'), escapeHtml(String(disc.comparator_output ?? t('value_none')))],
        [t('disc_sound_event'), `<code class="value-code">${escapeHtml(disc.sound_event || '')}</code>`]
    ];
    if (disc.youtube) {
        rows.push(['YouTube', `<a href="${escapeHtml(disc.youtube).replace(/"/g, '&quot;')}" target="_blank" rel="noopener noreferrer"><i class="fab fa-youtube"></i> ${escapeHtml(t('disc_youtube'))}</a>`]);
    }
    const heading = [disc.artist, disc.title].filter(Boolean).join(' – ');
    box.innerHTML = `
        <h4 class="disc-info-title"><i class="fas fa-compact-disc"></i> ${escapeHtml(heading)}</h4>
        <table class="disc-info-table">
            ${rows.map(([label, value]) => `<tr><th scope="row">${escapeHtml(label)}</th><td>${value}</td></tr>`).join('')}
        </table>`;
}

// ===== MODAL FUNCTIONS =====
// Links like items.html?item=minecraft:rotten_flesh (from the mobs DB) open that item.
function openItemFromUrl() {
    const id = new URLSearchParams(location.search).get('item');
    if (!id) return;
    const item = getAllItemsDeduped().find(i => i.id === id || i.java_item_id === id);
...
function setModalBasicInfo(item) {
    modalItemName.textContent = item.name || t('value_unknown');
    modalItemIcon.style.backgroundImage = `url('${item.icon || '/assets/img/mobs/default/default.png'}')`;
    modalItemId.textContent = item.id || t('value_none');
...
function setModalExtendedInfo(item) {
    modalItemVersion.textContent = item.version || t('value_unknown');
    modalItemNumericID.textContent = item.numeric_id !== undefined ? item.numeric_id : t('value_none');
...
// Animated textures are vertical strips of frames; CSS steps through them.
function applyTextureBackground(el, texture) {
    el.style.backgroundImage = texture.file ? `url('${texture.file}')` : '';
    const frames = Number(texture.frames) || 0;
    el.classList.toggle('is-animated', frames > 1);
    if (frames > 1) {
        el.style.setProperty('--frames', frames);
        el.style.setProperty('--frame-ms', (Number(texture.frametime) || 1) * 50);
    }
}
...
function setModalTexture(item) {
    ...
    if (!textures.length) {
        ...
        applyTextureBackground(modalItemTextureIcon, {});
        return;
    }

    applyTextureBackground(modalItemTextureIcon, textures[0]);
    ...
    textures.forEach((texture, index) => {
        ...
        textureCard.className = index === 0 ? 'texture-card active' : 'texture-card';

        const previewImg = document.createElement('div');
        previewImg.className = 'texture-thumb';
        applyTextureBackground(previewImg, texture);

        const nameEl = document.createElement('div');
        nameEl.className = 'texture-name';
        nameEl.textContent = meta.name;

        textureCard.append(previewImg, nameEl);
        if (meta.description) {
            const descEl = document.createElement('div');
            descEl.className = 'texture-description';
            descEl.textContent = meta.description;
            textureCard.appendChild(descEl);
        }

        textureCard.addEventListener('click', () => {
            applyTextureBackground(modalItemTextureIcon, texture);
            modalItemTexturesContainer.querySelectorAll('.texture-card').forEach(card => card.classList.remove('active'));
            textureCard.classList.add('active');
            playClickSound();
        });
...
// ===== SOUND TIME (elapsed / total) =====
function formatSoundTime(seconds, short) {
    if (!Number.isFinite(seconds) || seconds < 0) return '–:––';
    if (short) return seconds.toFixed(1).replace('.', lang === 'de' ? ',' : '.');
    const s = Math.floor(seconds);
    return `${Math.floor(s / 60)}:${String(s % 60).padStart(2, '0')}`;
}

function setSoundTime(row, current, duration) {
    const el = row.querySelector('.sound-time');
    if (!el) return;
    // Sounds under 10 s in tenths of a second, e.g. "0,3 / 0,6 s".
    const short = Number.isFinite(duration) && duration < 10;
    el.textContent = short
        ? `${formatSoundTime(Math.min(current, duration), true)} / ${formatSoundTime(duration, true)} s`
        : `${formatSoundTime(current)} / ${formatSoundTime(duration)}`;
}

function resetSoundRow(row) {
    row._audio = null;
    row.classList.remove('playing');
    setSoundTime(row, 0, Number(row.dataset.duration));
}

// The row shows the time of the audio it started last.
function followSound(row, audio) {
    row._audio = audio;
    row.classList.add('playing');
    const duration = () => !row.dataset.fixed && Number.isFinite(audio.duration) ? audio.duration : Number(row.dataset.duration);
    const update = () => { if (row._audio === audio) setSoundTime(row, audio.currentTime, duration()); };
    audio.addEventListener('loadedmetadata', () => {
        if (row._audio === audio && !row.dataset.fixed && Number.isFinite(audio.duration)) row.dataset.duration = audio.duration;
        update();
    });
    audio.addEventListener('timeupdate', update);
    audio.addEventListener('ended', () => { if (row._audio === audio) resetSoundRow(row); });
    update();
}

function discSoundLength(item, sound) {
    const disc = item.music_disc;
    return disc && sound.event && disc.sound_event?.endsWith(':' + sound.event) ? disc.length_seconds : undefined;
}

// Lengths of the other rows are read from the files once the Sounds tab is open.
function probeSoundDurations() {
    soundsContainer?.querySelectorAll('.sound-item:not([data-duration])').forEach(row => {
        const sound = currentItem?.sounds?.[Number(row.dataset.soundIndex)];
        const file = sound && soundFiles(sound)[0];
        if (!file || row.dataset.probing) return;
        row.dataset.probing = '1';
        const probe = new Audio();
        probe.preload = 'metadata';
        probe.addEventListener('loadedmetadata', () => {
            if (!Number.isFinite(probe.duration) || row.dataset.duration) return;
            row.dataset.duration = probe.duration;
            if (!row._audio) setSoundTime(row, 0, probe.duration);
        }, { once: true });
        probe.src = file;
    });
}

function startSound(file, row) {
    const audio = new Audio(file);
    audio.volume = 0.5;
    audio.play().catch(()=>{});
    trackSound(audio);
    if (row) followSound(row, audio);
}
...
function loadItemSounds(item) {
    ...
    sounds.forEach((sound, idx) => {
        ...
        soundItem.innerHTML = `
            ...
            <div class="sound-info">
                ...
                <div class="sound-time"></div>
            </div>
            ${files.length > 1 ? `<div class="sound-variants">
                <span class="sound-duration">${escapeHtml(t('modal_sound_variants', { n: files.length }))}</span>
                <div class="sound-variant-list">${files.map((file, i) => {
        ...
        `;
        const discLength = discSoundLength(item, sound);
        if (discLength) {
            // The game's length; a browser may only estimate it for a large file.
            soundItem.dataset.duration = discLength;
            soundItem.dataset.fixed = '1';
        }
        setSoundTime(soundItem, 0, Number(discLength));
        ...
        const chosenFile = (target) => {
            ...
            return btn ? files[Number(btn.dataset.variant)] : undefined;
        };
        ...
        soundItem.addEventListener('mousedown', (e) => {
            ...
            loop = setInterval(() => playSoundOnce(sound, soundItem, file), 100);
        });
    ...
    if (document.getElementById('soundsTab')?.classList.contains('active')) probeSoundDurations();
}
function playSoundOnce(sound, element, file) {
    file = file || randomSoundFile(sound);
    if(!soundEnabled || !file) return;
    try {
        startSound(file, element);
...
// Remembers playing audio so "Stop all" also stops single rows (e.g. a music disc).
function trackSound(audio) {
...
function playAllSounds() {
    ...
    sounds.forEach((sound, idx) => {
        ...
        if(!file) return;
        try {
            startSound(file, soundsContainer?.querySelector(`.sound-item[data-sound-index="${idx}"]`));
        } catch(e) {}
    });
}
function stopAllSounds() {
    ...
    activeSounds = [];
    soundsContainer?.querySelectorAll('.sound-item').forEach(resetSoundRow);
}
...
function initEventListeners() {
    ...
    document.querySelectorAll('.item-tab').forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            if (tabId === 'sounds') probeSoundDurations();
            playClickSound();
        });
    });
