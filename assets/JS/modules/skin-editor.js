            ...
            <div class="app-container">
                    <div class="toolbar">
                            <div class="toolbar-row toolbar-shell">
                                <div class="tool-group tool-group-files"><span class="tool-group-label">📂 ${lang === 'de' ? 'Datei' : 'File'}</span>
                ...
                <div class="main-content">
                    <div class="left-panel">
                                <div class="layer-controls"><div class="panel-title">🧍 ${lang === 'de' ? 'Modell typ' : 'Model Type'}</div>
                            <div class="layer-row">
                        ...
                        <div class="layer-controls"><div class="panel-title">📋 ${lang === 'de' ? 'Anzeigeoptionen' : 'Display Options'}</div>
                            <div class="layer-row">
                        ...
                        <div class="layer-controls"><div class="panel-title">👤 ${lang === 'de' ? 'Körperteile' : 'Body Parts'}</div>
                        ...
                        <div class="layer-controls"><div class="panel-title">🗺️ 2D UV Editor</div>
                            ...
                            <div class="uv-hint">${lang === 'de' ? 'Male direkt auf der Skin-Map (aktive Ebene: Body/Outer)' : 'Paint directly on the skin map (active layer: Body/Outer)'}</div>
                        </div>
                    </div>
                    ...
                    <div class="right-panel">
                        <div class="layer-controls"><div class="panel-title">🎯 ${lang === 'de' ? 'Farben' : 'Colors'}</div>
                            <div id="color-palette"></div>
                            <div class="color-picker-container">
                                <span>${lang === 'de' ? 'Aktuelle Farbe:' : 'Current Color:'}</span>
                        ...
                        <div class="layer-controls"><div class="panel-title">✨ MC-Craft Extras</div>
                            ...
                            <div class="uv-hint">${lang === 'de' ? 'Aether Mirror spiegelt deine Pinselstriche horizontal auf derselben Ebene. Overlay Focus blendet den Body im Edit-Modus zurück.' : 'Aether Mirror mirrors your brush strokes horizontally on the same layer. Overlay Focus hides the body in edit mode.'}</div>
                        </div>
                       <div class="layer-controls"><div class="panel-title">🔧 ${lang === 'de' ? 'Aktionen' : 'Actions'}</div>
...
function pickColorAtTexture(x, y) {
    ...
    showToast(lang === 'de' ? 'Farbe' : 'Color', t('toast_color_picked', {color}));
}
...
function loadSteveSkin(saveHistory = true) {
    ...
    img.onerror = () => { createEmptySkin(); showToast('Info', lang === 'de' ? 'Leerer Skin erstellt' : 'Empty skin created'); };
...
function toggleMirrorPainting() {
    ...
    showToast('Mirror', mirrorPainting ? (lang === 'de' ? 'Aether Mirror aktiviert' : 'Aether Mirror enabled') : (lang === 'de' ? 'Aether Mirror deaktiviert' : 'Aether Mirror disabled'));
}
...
function toggleLayerVisibility(layer) {
    ...
    updateGridOverlay();
    updateModeIndicators();
    showToast(lang === 'de' ? 'Ebene' : 'Layer', t('toast_layer_changed', {layer}));
}
...
function toggleBodyPart(part) {
    ...
    showToast(lang === 'de' ? 'Körperteil' : 'Body Part', t('toast_part_toggled', {part, state: bodyPartsVisible[part] ? (lang === 'de' ? 'sichtbar' : 'visible') : (lang === 'de' ? 'versteckt' : 'hidden')}));
}
...
function setModelType(type) {
    ...
    updatePlayerModelStatus();
    updateModeIndicators();
    showToast(lang === 'de' ? 'Modell' : 'Model', `${lang === 'de' ? 'Modelltyp' : 'Model type'}: ${type === 'slim' ? 'Slim (3px)' : 'Classic (4px)'}`);
}
...
async function getPlayerSkinData(input) {
    const player = await MCCraftAPI.getPlayer(input);
    let modelType = null;
    if (typeof player.isSlim === 'boolean') {
        modelType = player.isSlim ? 'slim' : 'classic';
    } else if (player.modelType === 'slim' || player.modelType === 'classic') {
        modelType = player.modelType;
    }

    return {
        username: player.username || input.trim(),
        skinUrl: player.skinUrl ? MCCraftAPI.getPlayerSkinUrl(input) : STEVE_SKIN_URL,
        modelType
    };
}
...
async function loadPlayerSkin() {
    ...
    try {
        ...
        showToast(lang === 'de' ? 'Modell' : 'Model', t('toast_model_detected', {
        ...
        let title = 'Skin';
        let message = t('toast_skin_load_failed');

        switch (code) {
            case 'PLAYER_NOT_FOUND':
                title = t('toast_err_player_not_found_title');
                message = t('toast_err_player_not_found_msg', { name: value });
                break;
...
function importSkin(e) {
    ...
    img.onload = () => {
        try {
            ...
            showToast('Import', lang === 'de' ? 'Nur 64x64 oder 64x32 PNG-Skins werden unterstützt.' : 'Only 64x64 or 64x32 PNG skins are supported.', 'error');
        }
        URL.revokeObjectURL(img.src);
    };
    img.onerror = () => {
        showToast('Import', lang === 'de' ? 'Die Skin-Datei konnte nicht geladen werden.' : 'The skin file could not be loaded.', 'error');
        URL.revokeObjectURL(img.src);
    };
...
function applyColorCode() {
    ...
    if (/^#[0-9A-F]{6}$/i.test(code)) {
        ...
        showToast(lang === 'de' ? 'Farbe' : 'Color', t('toast_color_picked', {color: currentColor}));
    } else {
        showToast(lang === 'de' ? 'Fehler' : 'Error', t('toast_invalid_color'), 'error');
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    updateModelButtons();
    updateModeIndicators();
    // Header chrome (theme switcher, sound toggle, mobile drawer, back-to-top and
    // the click sound) is wired by main.js. The copy that used to live here bound
    // a SECOND click handler to #themeBtn, so one click toggled the dropdown twice
    // and it never appeared - and the sound button toggled twice, cancelling out.
});
