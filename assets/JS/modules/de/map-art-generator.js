// Map-Art-Generator page (Deutsch) — UI glue only.
...
const T = {
    loader_text: "Map-Art-Generator wird geladen...",
    loader_text2: "Block-Datenbank wird initialisiert...",
    loader_text3: "Fast fertig...",
    toast_loaded_title: "Bereit!",
    toast_loaded_message: "Lade ein Bild hoch, um loszulegen.",
    ...
    status_idle: "Noch kein Bild geladen.",
    status_reading: "Bild wird gelesen...",
    status_ready: "Bereit zum Generieren.",
    status_computing: "Berechne Blöcke... (kann bei großen Karten etwas dauern)",
    status_done: "Fertig! {count} Blöcke, Höhe {height}. Bereit als {format} zum Download.",
    toast_generated_title: "Fertig!",
    toast_generated_message: "{count} Blöcke berechnet.",
    toast_error_title: "Fehler",
    error_UNSUPPORTED_INPUT: "Diese Bilddatei kann nicht gelesen werden.",
    error_IMAGE_DECODE_FAILED: "Das Bild konnte nicht gelesen werden – ist es wirklich eine Bilddatei (PNG/JPG/WebP)?",
    error_BLOCK_COLORS_LOAD_FAILED: "Die Block-Farbtabelle konnte nicht geladen werden. Bitte Seite über einen Server (nicht als lokale Datei) öffnen.",
    error_VOLUME_TOO_LARGE: "Das Ergebnis wäre zu groß für diesen Browser-Generator. Bitte eine kleinere Kartengröße wählen.",
    error_NO_BLOCKS_SELECTED: "Bitte mindestens einen Block in der Blockauswahl anhaken.",
    error_MAPDAT_NEEDS_ZIP: ".dat als Einzeldatei geht nur bei genau 1×1 Kartengröße. Bei größeren Karten bitte den ZIP-Download nutzen.",
    error_UNKNOWN: "Ein unerwarteter Fehler ist aufgetreten.",
    picker_summary: "{selected} von {total} Blöcken ausgewählt",
    toast_zip_title: "ZIP erstellt!",
    toast_zip_message: "{count} Karten-Dateien im ZIP."
};

const FORMAT_LABELS = { schematic: '.schematic', schem: '.schem', litematic: '.litematic', nbt: '.nbt', mapdat: '.dat (Minecraft-Karte)' };
...
function refineCategory(block) {
    const base = block.category || 'Sonstiges';
...
const MAP_COLOR_NAMES = {
    1: 'Gras', 2: 'Sand', 3: 'Wolle', 4: 'Feuer', 5: 'Eis', 6: 'Metall', 7: 'Pflanze', 8: 'Schnee',
    9: 'Ton', 10: 'Erde', 11: 'Stein', 12: 'Wasser', 13: 'Holz', 14: 'Quarz',
    15: 'Orange', 16: 'Magenta', 17: 'Hellblau', 18: 'Gelb', 19: 'Hellgrün', 20: 'Pink',
    21: 'Grau', 22: 'Hellgrau', 23: 'Cyan', 24: 'Lila', 25: 'Blau', 26: 'Braun', 27: 'Grün', 28: 'Rot', 29: 'Schwarz',
    30: 'Gold', 31: 'Diamant', 32: 'Lapislazuli', 33: 'Smaragd', 34: 'Podsol', 35: 'Nether',
    36: 'Weiße Terrakotta', 37: 'Orangene Terrakotta', 38: 'Magentafarbene Terrakotta', 39: 'Hellblaue Terrakotta',
    40: 'Gelbe Terrakotta', 41: 'Hellgrüne Terrakotta', 42: 'Rosafarbene Terrakotta', 43: 'Graue Terrakotta',
    44: 'Hellgraue Terrakotta', 45: 'Cyanfarbene Terrakotta', 46: 'Violette Terrakotta', 47: 'Blaue Terrakotta',
    48: 'Braune Terrakotta', 49: 'Grüne Terrakotta', 50: 'Rote Terrakotta', 51: 'Schwarze Terrakotta',
    52: 'Karmesinnetherrack', 53: 'Karmesinstamm', 54: 'Karmesinholz', 55: 'Verzerrter Netherrack',
    56: 'Verzerrter Stamm', 57: 'Verzerrtes Holz', 58: 'Verzerrter Wärzenblock',
    59: 'Tiefenschiefer', 60: 'Roheisen', 61: 'Leuchtermoos'
};
const SHADE_INFO = [
    { id: 0, mult: 180, label: 'Stufe 0' },
    { id: 1, mult: 220, label: 'Stufe 1' },
    { id: 2, mult: 255, label: 'Stufe 2 (normal)' },
    { id: 3, mult: 135, label: 'Stufe 3' }
];
...
function renderBlockColorTable(blocks, query) {
    ...
    const filtered = q
        ? blocks.filter((b) => b.nameDe.toLowerCase().includes(q) || b.id.toLowerCase().includes(q))
        : blocks;
    ...
    if (filtered.length === 0) {
        container.innerHTML = '<p class="ma-hint">Keine Blöcke gefunden.</p>';
        return;
    }

    container.innerHTML = orderedCats.map((cat) => {
        const items = byCategory.get(cat).sort((a, b) => a.nameDe.localeCompare(b.nameDe, 'de'));
        const rows = items.map((b) => {
            const hex = '#' + toHex(b.r) + toHex(b.g) + toHex(b.b);
            return `<tr><td><img src="${b.icon}" alt="" loading="lazy" width="28" height="28"></td>` +
                `<td>${b.nameDe}</td><td><span class="ma-swatch" style="background:${hex}"></span><code>${hex}</code></td>` +
                `<td>${b.r}, ${b.g}, ${b.b}</td></tr>`;
        }).join('');
        return `<h4 class="ma-block-color-cat">${cat} <span class="ma-hint">(${items.length})</span></h4>` +
...
const VERSION_PRESETS = [
    ...
    { value: '13800', label: '1.8 (z. B. 1.8.9)' },
    ...
    { value: '999999', label: 'Aktuell (keine Einschränkung)' }
];
...
async function loadBlockNameLookup() {
    ...
    await Promise.all(ITEM_CATEGORY_FILES.map(async (cat) => {
        try {
            const resp = await fetch(`/assets/JS/items/de_${cat}.json`);
            if (!resp.ok) return;
...
async function buildBlockPicker() {
    ...
    VERSION_PRESETS.forEach((p) => {
        ...
        if (p.label.startsWith('Aktuell')) opt.selected = true;
    ...
    for (const cat of orderedCategories) {
        ...
        head.innerHTML = `<input type="checkbox" class="ma-picker-cat-toggle" checked title="Ganze Kategorie an-/abwählen"><span>${cat}</span> <span class="ma-picker-cat-count">(${blocks.length})</span> <i class="fas fa-chevron-down"></i>`;
...
async function renderMaterials(materialCountsEntries) {
    ...
    for (const [blockId, count] of sorted) {
        ...
        row.innerHTML = `
            ...
            <span class="ma-material-count">${count.toLocaleString('de-DE')}</span>
        `;
...
function buildMaterialsTxt() {
    ...
    for (const { name, count } of lastMaterialsExport) {
        lines.push(`${name.padEnd(maxLen)}  ${count.toLocaleString('de-DE')}`);
    }
    lines.push('', `Gesamt: ${currentTotalBlocks.toLocaleString('de-DE')} Blöcke`);
    return lines.join('\n');
}
...
async function handleGenerate() {
    ...
    try {
        ...
        previewMeta.textContent = `${result.cols} × ${result.rows} Pixel · Höhe: ${result.heightRange} Blöcke · ${currentTotalBlocks.toLocaleString('de-DE')} Blöcke gesamt`;

        populateTargetFormats();
        convertBtn.disabled = false;
        showToast(t('toast_generated_title'), t('toast_generated_message', { count: currentTotalBlocks.toLocaleString('de-DE') }));
        playLevelUpSound();
...
async function prepareDownload() {
    ...
    try {
        ...
        setStatus(t('status_done', {
            count: currentTotalBlocks.toLocaleString('de-DE'),
    ...
    try {
        ...
        if (fmt === 'mapdat') {
            ...
            entries = await Promise.all(tiles.map(async (tile) => {
                ...
                return { name: `karte${tile.index}${window.StructureConverter.FORMATS[fmt].ext}`, bytes: result.bytes };
            }));
        }
        const zipBytes = window.MapArtGenerator.buildZip(entries);
        downloadBlob(zipBytes, baseFilename() + '-karten.zip');
