...
const T = {
    ...
    toast_welcome_title: "Chibi Skin geladen!",
    toast_welcome_message: "Entdecke das Chibi Skin von MC-Craft.",
    ...
    // Loader
    loader_text1_chibi: "Chibi Skin Maker wird geladen...",
    loader_text2: "Skin Renderer wird vorbereitet...",
    loader_text3: "Chibi Modelle werden geladen...",
    loader_text4: "Werkzeuge werden vorbereitet...",
    ...
    // Alte/optionale Referenzen: null ist erlaubt.
...
function playClickSound() {
    ...
    try {
        ...
        // Sound darf niemals die Seite kaputtmachen.
    }
}
...
const ANIM_FORMATS = {
  ...
  apng: {
    label: 'APNG', ext: 'png',
    note: 'Signal und LINE Aufkleber, und Discord bei 320 × 320. Kleiner als GIF, mit weichen Kanten.',
  },
  webm: {
    label: 'WebM', ext: 'webm',
    note: 'Telegram Video-Sticker: 512 px auf einer Seite, unter 256 KB, bis zu 3 Sekunden.',
  },
};
...
class ChibiSkinMaker {
  ...
  setupEventListeners() {
    ...
    if (this.farLegShadeToggle) this.farLegShadeToggle.addEventListener('change', (e) => {
      ...
      this._refreshSkinTexture();
      this.setStatus(this.farLegShade ? 'Hinteres Bein wird schattiert.' : 'Hinteres Bein wird wie das vordere beleuchtet.');
    });
    if (this.outlinePerPartToggle) this.outlinePerPartToggle.addEventListener('change', (e) => {
      ...
      this._refreshSkinTexture();
      this.setStatus(this.outlinePerPart ? 'Kontur um jedes Teil.' : 'Kontur um die ganze Figur.');
    });
    if (this.outlineAutoToggle) this.outlineAutoToggle.addEventListener('change', (e) => {
      ...
      this._refreshSkinTexture();
      this.setStatus(this.outlineAuto ? 'Konturfarbe folgt der Kunst.' : 'Konturfarbe: benutzerdefiniert.');
    });
    const claimColor = () => {
    ...
    if (this.bgImageInput) this.bgImageInput.addEventListener('change', (e) => {
      ...
      reader.onload = async () => {
        ...
        this.setStatus('Hintergrundbild gesetzt – Transparenz deaktiviert.');
      };
    ...
    this.viewerContainer.addEventListener('wheel', (e) => {
      ...
      if ((this.bgShape || 'none') !== 'none') {
        ...
        this._draw();
        this.setStatus('Figur ' + next + 'x' + (next === auto ? ' (passend)' : '') + ' – ziehen zum Verschieben, Doppelklick zum Zurücksetzen.');
        return;
      }
      ...
      this._draw();
      this.setStatus('Zoom ' + next.toFixed(next % 1 ? 2 : 0) + 'x – Doppelklick zum Zurücksetzen.');
    }, { passive: false });
    this.viewerContainer.addEventListener('dblclick', () => {
      ...
      this._draw();
      this.setStatus('Zoom und Position zurückgesetzt.');
    });
    let panFrom = null;
    ...
    const movePinch = () => {
      ...
      if (pinch.badge) {
        ...
        this.setStatus('Figur ' + eff + 'x' + (eff === fit ? ' (passend)' : '') + ' – ziehen zum Verschieben, Doppelklick zum Zurücksetzen.');
      } else {
        const auto = this._viewBaseScale();
        ...
        if (!this.viewZoom) { this.viewPanX = 0; this.viewPanY = 0; }
        this.setStatus('Zoom ' + eff.toFixed(eff % 1 ? 2 : 0) + 'x – Doppelklick zum Zurücksetzen.');
      }
      this._draw();
    };
    ...
    if (confirmOk) confirmOk.addEventListener('click', () => {
      this.closeDialog();
      this.resetAll();
      this.setStatus('Alles zurückgesetzt.');
    });
...
async loadSkinFromUsername() {
    ...
    try {
        ...
        if (!player || !player.skinUrl) {
            throw new Error('Spieler oder Skin wurde nicht gefunden.');
        }
        ...
        if (!resp.ok) {
            throw new Error('Skin konnte nicht geladen werden.');
        }
        ...
        const dataUri = await new Promise((resolve, reject) => {
            ...
            reader.onerror = () => reject(
                new Error('Skin konnte nicht gelesen werden.')
            );
        ...
        await new Promise((resolve, reject) => {
            ...
            img.onerror = () => reject(
                new Error('Skin konnte nicht geladen werden.')
            );
        ...
        if (!this._validSkinSize(img)) {
            throw new Error(
                `Die API lieferte ein ungültiges Skin-Bild (${img.width}x${img.height}).`
            );
        }
        ...
        this.setStatus(
            ...
            ' über die MC-Craft API.'
        );

    } catch (err) {
        console.error('MC-Craft Skin API Fehler:', err);

        let message = 'Skin konnte nicht geladen werden.';
        ...
        if (
            ...
            message = 'Dieser Minecraft-Spieler wurde nicht gefunden.';
        } else if (err && err.code === 'TIMEOUT') {
            message = 'Die MC-Craft API antwortet gerade zu langsam.';
        } else if (err && err.code === 'NETWORK_ERROR') {
            message = 'Die MC-Craft API ist momentan nicht erreichbar.';
  ...
  async _applyBodyType(mode) {
    ...
    if (mode === 'auto') {
      ...
      this.setStatus(resolved
        ? 'Auto: ' + (resolved === 'alex' ? 'schmale Arme erkannt, Alex.' : 'breite Arme erkannt, Steve.')
        : (this.currentSkinImage ? 'Auto: behalte ' + this.currentModelType + '.' : 'Auto: Körpertyp folgt dem Skin.'));
      return;
    }

    await this.loadModel(mode, { clearUndo: false });
    this._refreshSkinTexture();
    this.setStatus('Körpertyp auf ' + (mode === 'alex' ? 'Alex' : 'Steve') + ' festgelegt.');
  }
  ...
  handleDroppedFile(e) {
    ...
    if (file.type === 'image/png' || /\.png$/i.test(file.name)) {
      ...
      this.showError('Bitte eine Skin-PNG ablegen.');
    }
  }
  ...
  loadSkinFromFile(file) {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        if (ratio !== 1 && ratio !== 2) {
          this.showError('Skin muss quadratisch sein (64x64, 128x128, ...) oder ein legacy 2:1-Sheet (64x32, 128x64, ...).');
          return;
        }
        if (img.width < 64 || (img.width & (img.width - 1)) !== 0) {
          this.showError('Skin-Breite muss eine Zweierpotenz sein: 64, 128, 256, 512 ...');
          return;
        }
        this.pushUndo(preState);
  ...
  downloadSkin() {
    ...
    if (this._skinFileBlob && this._skinVerbatim) {
      ...
      this._flashBtn(this.downloadSkinBtn, 'Gespeichert!', true);
      return;
    }
    const canvas = document.createElement('canvas');
    ...
    canvas.toBlob((blob) => {
      if (!blob) { this._flashBtn(this.downloadSkinBtn, 'Fehler', false); return; }
  ...
  setView(key) {
    ...
    this.setStatus('Ansicht: ' + (btn ? btn.textContent : key) + '.');
  }
  ...
  setDetail(key) {
    ...
    this._refreshSkinTexture();
    this.setStatus('Auflösung: ' + (next === 'fine' ? 'hochauflösend' : 'nativ') + '.');
  }
  ...
  _syncDetailButtons() {
    ...
    for (const b of this.detailButtons || []) {
      ...
      b.title = off ? 'Nativ ist der 64x64-Look; dieser Skin ist HD, daher wird hochauflösend gerendert.'
        : b.dataset.detail === 'blocky'
          ? 'Der Look der Referenz: ein Gesichtspixel ist 3x3'
          : 'Ein Pixel pro Skin-Texel, HD-Skins werden größer dargestellt';
    }
  ...
  setStyle(key) {
    ...
    this._writeStyleToURL();
    this._refreshSkinTexture();
    this.setStatus('Stil: ' + (next === 'live' ? 'Minecraft Live' : 'Klassisch') + '.');
  }
  ...
  _syncStyleButtons() {
    ...
    const gate = (btns, liveAllowed, classicAllowed = null) => {
      ...
      for (const b of btns || []) {
        ...
        b.disabled = off;
        b.classList.toggle('is-unavailable', off);
        if (off) b.title = live ? 'Der Minecraft-Live-Stil zeichnet dies noch nicht.'
          : 'Der Klassik-Stil zeichnet dies noch nicht.';
  ...
  setBust(key) {
    ...
    this._syncPoseButtons();
    this._syncViewButtons();
    this.setStatus('Bust: ' + (next ? (ChibiSkinMaker.BUST_LABEL[next] || next) : 'ganzer Körper') + '.');
  }
  ...
  setLayerShadow(on) {
    ...
    this._refreshSkinTexture();
    this.setStatus('Ebenenschatten: ' + (next ? 'ein' : 'aus') + '.');
  }
  ...
  setAnim(key) {
    ...
    this._syncAnimButtons();
    this._updateGifInfo();
    this.setStatus(key === 'none' ? 'Animation aus.' : 'Animation: ' + CHIBI_ANIMS[key].label + ' – exportiere sie über das GIF-Panel.');
  }
  ...
  clearSkinStatus() {
    this.skinStatusText.textContent = this.currentModelType === 'alex' ? 'Alex (Standard)' : 'Steve (Standard)';
  ...
  setupValueScrubbing() {
    ...
    for (const input of inputs) {
      ...
      input.title = 'Zum Anpassen nach links/rechts ziehen';
  ...
  setupPanels() {
    ...
    const titles = {
      skin: 'Skin', look: 'Aussehen', pose: 'Pose', anim: 'Animation',
      camera: 'Kamera', parts: 'Teile',
    };
  ...
  setupPreviewPane() {
      ...
      if (rect.top <= 80) {
        if (!card.classList.contains('pinned')) {
          ...
          if (!placeholder) {
            ...
            placeholder.textContent = 'Hier ablegen zum Andocken';
    ...
    pinBtn.addEventListener('click', () => {
      ...
      if (isPinEnabled) {
        ...
        pinBtn.setAttribute('title', 'Vorschau lösen');
        pinBtn.setAttribute('aria-label', 'Vorschau lösen');
        window.addEventListener('scroll', updatePinPosition);
        window.addEventListener('resize', handleResize);
        updatePinPosition();
      } else {
        pinBtn.classList.remove('active');
        pinBtn.setAttribute('title', 'Vorschau anheften');
        pinBtn.setAttribute('aria-label', 'Vorschau anheften');
  ...
  async applyConfig(input) {
    ...
    try {
      ...
      if (c.skin && c.skin.dataUri) {
        ...
        if (!this._validSkinSize(img)) throw new Error(`Der Skin in der Config hat ungültige Maße (${img.width}x${img.height}).`);
        this.applySkinTexture(img);
  ...
  setupConfigModal() {
    ...
    const readFile = (file) => {
      ...
      reader.onerror = () => setStatus('Die Datei konnte nicht gelesen werden.', 'error');
    ...
    const importText = async (text, name) => {
      ...
      let cfg;
      try { cfg = JSON.parse(text); } catch { setStatus('Die Datei ist kein gültiges JSON.', 'error'); return; }
      if (!cfg || typeof cfg !== 'object' || Array.isArray(cfg)) { setStatus('Eine Config muss ein JSON-Objekt sein.', 'error'); return; }
      try {
        setStatus('Wird angewendet...', '');
        await this.applyConfig(cfg);
        refreshExport();
        setStatus('Geladen ' + (name || 'Config'), 'success');
        this._configCloseTimer = setTimeout(() => { if (this.openDialogEl === overlay) this.closeDialog(); }, 700);
      } catch (err) { console.error('Config-Import fehlgeschlagen:', err); setStatus('Diese Config konnte nicht angewendet werden.', 'error'); }
    };
    ...
    if (copyBtn) copyBtn.addEventListener('click', async () => {
      ...
      this._flashConfigBtn(copyBtn, ok ? 'Kopiert!' : 'Fehlgeschlagen', ok);
    });
    if (downloadBtn) downloadBtn.addEventListener('click', () => {
      ...
      this._flashConfigBtn(downloadBtn, 'Gespeichert!', true);
    });
  }
