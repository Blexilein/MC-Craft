...
const SERVICES = [
    ...
    { id: 'auth-mojang-com', name: 'Mojang Auth', url: 'https://auth.mojang.com/', icon: 'fas fa-lock', desc: { de: 'Klassischer Mojang-Authentifizierungsdienst', en: 'Classic Mojang authentication service' }, probe: () => probeCors('https://auth.mojang.com/') },
    { id: 'skins-mc-net', name: 'Minecraft Skin Server', url: 'https://skins.minecraft.net/', icon: 'fas fa-person', desc: { de: 'Früherer Skin-Dienst', en: 'Former skin service' }, probe: () => probeCors('https://skins.minecraft.net/') },
    { id: 'authserver-mojang', name: 'Mojang Auth Server', url: 'https://authserver.mojang.com/', icon: 'fas fa-key', desc: { de: 'Yggdrasil-Authentifizierungsserver', en: 'Yggdrasil authentication server' }, probe: () => probeCors('https://authserver.mojang.com/') },
    ...
    { id: 'status-mojang', name: 'Mojang Status API', url: 'https://status.mojang.com/', icon: 'fas fa-heart-pulse', desc: { de: 'Frühere zentrale Mojang-Statusabfrage', en: 'Former central Mojang status endpoint' }, probe: () => probeCors('https://status.mojang.com/check') },
...
function getTranslation(key, placeholders) {
    let text = (translations[currentLang] || translations.de)[key] || translations.de[key] || key;
    if (placeholders) {
...
function getThemeName(theme) {
    return getTranslation({ overworld: 'theme_overworld', nether: 'theme_nether', end: 'theme_end' }[theme] || 'theme_overworld');
}
...
function toggleSound() {
    ...
    updateSoundIcon();
    playClickSound();
    showToast(getTranslation('toast_sound_title'), getTranslation(soundEnabled ? 'toast_sound_on' : 'toast_sound_off'), 'info');
}
...
function toggleLanguage() {
    ...
    updateLanguage();
    playClickSound();
    showToast(getTranslation('toast_language_title'), getTranslation(currentLang === 'de' ? 'toast_language_de' : 'toast_language_en'), 'info');
}
...
function updateLanguage() {
    ...
    document.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        if (el.children.length === 0 || t.includes('<')) {
            ...
            if (node && node.nodeType === Node.TEXT_NODE) node.textContent = ' ' + t;
        }
    });
    ...
    document.querySelectorAll('.api-card').forEach(card => {
        ...
        if (svc) {
            ...
            if (descEl) descEl.textContent = svc.desc[currentLang] || svc.desc.de;
        }
    });
    ...
    document.querySelectorAll('.api-status-badge').forEach(badge => {
        ...
        if (label) {
            label.textContent = getTranslation(BADGE_KEYS[status]);
        }
    });

    document.title = getTranslation('site_title_apistatus');
}
...
function initLoader() {
    ...
    const texts = ['apistatus_loader_text1', 'loader_text2', 'loader_text3', 'loader_text4'].map(k => getTranslation(k));
...
function initThemeSwitcher() {
    ...
    document.querySelectorAll('.theme-option, .theme-option-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            showToast(getTranslation('toast_theme_changed'), getTranslation('toast_theme_to', { theme: getThemeName(theme) }), 'info');
        });
    });
}
...
function buildCard(svc) {
    ...
    card.innerHTML =
        ...
        '<div class="api-card-info">' +
            ...
            '<div class="api-card-desc">' + (svc.desc[currentLang] || svc.desc.de) + '</div>' +
        '</div>' +
        '<div class="api-status-badge checking" id="badge-' + svc.id + '">' +
            '<span class="status-dot"></span>' +
            '<span id="badge-label-' + svc.id + '">' + getTranslation('apistatus_checking_label') + '</span>' +
        '</div>';
    return card;
}
...
function updateCard(svc, status) {
    ...
    label.textContent = getTranslation(BADGE_KEYS[status] || BADGE_KEYS.unknown);
}
...
async function checkAll() {
    ...
    btn.disabled = true;
    document.getElementById('lastCheck').textContent = getTranslation('apistatus_checking');
    ...
    document.getElementById('lastCheck').textContent =
        getTranslation('apistatus_last_check') + ': ' +
