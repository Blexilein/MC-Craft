...
const translations = {
    de: {
        ...
        terms_card5_note: "Die Haftung für Vorsatz und grobe Fahrlässigkeit bleibt unberührt.",
        ...
        terms_card8_p2: "Durch die weiterführende Nutzung unserer Dienste nach Veröffentlichung von Änderungen erklären Sie sich mit den geänderten Bedingungen einverstanden.",
    ...
    en: {
        ...
        terms_card5_note: "Liability for intent and gross negligence remains unaffected.",
        ...
        terms_card8_p2: "By continuing to use our services after changes are published, you agree to the amended terms.",
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    initLanguageToggle();
    updateLanguage();
    checkTermsAcceptance(); // nach updateLanguage, damit der Button-Text ggf. korrigiert wird
});
...
function updateLanguage() {
    ...
    // Seitentitel
    document.title = getTranslation('site_title_terms');
}
...
function initLoader() {
    ...
    const texts = [
        getTranslation('loader_text1_terms'),
...
function initAcceptButton() {
    if (acceptBtn) {
        acceptBtn.addEventListener('click', () => {
            ...
            showToast(
                getTranslation('toast_accept_title'),
                getTranslation('toast_accept_message'),
                'success'
            );
            
            acceptBtn.innerHTML = '<i class="fas fa-check-circle"></i> ' + getTranslation('terms_accepted_btn');
...
function checkTermsAcceptance() {
    ...
    if (accepted === 'true' && acceptBtn) {
        ...
        if (acceptedDate) {
            ...
            showToast(
                getTranslation('toast_already_accepted'),
                `${getTranslation('toast_accepted_on')} ${date.toLocaleDateString(currentLang === 'de' ? 'de-DE' : 'en-US')}`,
                'info'
            );
        }
    }
