...
function copyCommand() {
    ...
    navigator.clipboard.writeText(text).then(() => {
        showToast(getTranslation('copy_success'), '', 'success');
    }).catch(() => {
        showToast(getTranslation('copy_error'), '', 'error');
    });
}
...
function updateLanguage() {
    ...
    document.title = getTranslation('day_night_title1')
}
...
function initLoader() {
    ...
    const texts = [
        getTranslation('loader_text1_day_night'),
