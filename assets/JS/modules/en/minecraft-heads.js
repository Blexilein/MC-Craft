...
(function () {
    ...
    function copyToClipboard(text) {
        navigator.clipboard.writeText(text).then(function () {
            if (typeof window.showToast === 'function') {
                window.showToast('Copied to clipboard', 'Success', 'success');
            }
        }).catch(function () {
            if (typeof window.showToast === 'function') {
                window.showToast('Copy failed', 'Error', 'error');
            }
        });
    }
    ...
    function headMatchesQuery(head, query) {
        ...
        return head.id.toLowerCase().indexOf(q) !== -1 ||
            head.nameEn.toLowerCase().indexOf(q) !== -1;
    }
    ...
    function renderHeadsGrid() {
        ...
        if (filtered.length === 0) {
            ...
            empty.textContent = 'No heads found.';
        ...
        if (headCountEl) {
            headCountEl.textContent = filtered.length + ' heads';
        }
    }
    ...
    function initThemeSwitcher() {
        ...
        document.querySelectorAll('.theme-option, .theme-option-btn').forEach(function (option) {
            option.addEventListener('click', function () {
                ...
                playClickSound();
                showToast('Theme changed', 'Switched to ' + (THEME_NAMES[theme] || theme));
            });
        });
    }
    ...
    var LOADER_TEXTS = [
        'Loading Head Database...',
        'Loading MHF heads...',
        'Loading blocks and symbols...',
        'Preparing logo heads...',
        'Almost done...'
    ];
    ...
    function runLoaderAnimation() {
        waitForSplashGone(function () {
                            playLevelUpSound();
                            showToast('Head database loaded!', '70 heads ready');
                        });
    }
    ...
    function init() {
        ...
        fetch(HEADS_URL).then(function (r) { return r.json(); }).then(function (data) {
            ...
            console.error('Failed to load head database:', err);
            if (headsGrid) {
                headsGrid.innerHTML = '<div class="no-results">Could not load data.</div>';
            }
        });
        runLoaderAnimation();
    }
