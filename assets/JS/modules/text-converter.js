...
function populateBannerSelect() {
    ...
    bannerTemplates.forEach((b, index) => {
        ...
        option.value = index;
        option.textContent = index === 0 ? getTranslation('banner_none') : b.name;
...
function updateLanguage() {
    ...
    // Seitentitel
    document.title = getTranslation('site_title_textconverter');
}
...
function initLoader() {
    ...
    const texts = [
        getTranslation('loader_text1_textconverter'),
...
function convertToMinecraft() {
    ...
    showToast(
        getTranslation('toast_convert_success_title'),
        getTranslation('toast_convert_success_message')
    );
    playClickSound();
}
...
function convertToNormal() {
    ...
    showToast(
        getTranslation('toast_convert_success_title'),
        getTranslation('toast_convert_normal_success')
    );
    playClickSound();
}
...
function copyMinecraftText() {
    ...
    if (!minecraftText || !minecraftText.value.trim()) {
        showToast(
            ...
            getTranslation('toast_error_no_minecraft')
        );
        return;
    }
    ...
    navigator.clipboard.writeText(minecraftText.value)
        .then(() => {
            showToast(
                getTranslation('toast_success_title'),
                getTranslation('toast_copy_minecraft_success')
            );
        ...
        .catch(err => {
            ...
            showToast(
                getTranslation('toast_error_title'),
                getTranslation('toast_copy_error')
            );
        });
    playClickSound();
}
...
function copyNormalText() {
    ...
    if (!normalText || !normalText.value.trim()) {
        showToast(
            getTranslation('toast_error_title'),
            getTranslation('toast_error_no_normal')
        );
        return;
    }
    ...
    navigator.clipboard.writeText(normalText.value)
        .then(() => {
            showToast(
                getTranslation('toast_success_title'),
                getTranslation('toast_copy_normal_success')
            );
...
window.addEventListener('error', function(e) {
    ...
    errorToast.innerHTML = `
        ...
        <div class="toast-content">
            <div class="toast-title">${getTranslation('toast_error_title')}</div>
            <div class="toast-message">${getTranslation('toast_error_message')}</div>
        </div>
    `;
